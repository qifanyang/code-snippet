...
public class LogEventHeader{
    ...
    private short flags;
    private byte[] extra_headers; //v4 FDE 中不会有extra_headers, stop event 中有改值
    ...
    public void parse(BinlogReader reader) throws IOException{
        ...
        this.setEventType(reader.readByte());
        ...
        parseExtraHeaders(reader);
    }

        //有的event 存在extra_header,
        //extra_header length, 如果存在event data, 里面会有headerLength, 可以计算出extra_header length
        //如果没有post-header和payload.那么根据event type 来确定
